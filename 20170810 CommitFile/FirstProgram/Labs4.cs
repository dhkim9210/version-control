      ...
      ElementSet elements )
    {
      ...
      foreach( ElementId id in uidoc.Selection.GetElementIds() )
      {
        ...
        List<string> a = new List<string>();
                ...
                System.IO.File.AppendAllText(savePath, e.Id.IntegerValue.ToString()+";", Encoding.Default);
                foreach ( Parameter p in e.Parameters )
        {
          ...
          a.Add( string.Format(
            ...
            ( read_only ? "only" : "write" ) ) );
                    ...
                    //string textValue = "";
                    System.IO.File.AppendAllText(savePath, string.Format(
            "Name={0}; Type={1}; Value={2}; ValueString={3}; read-{4}",
            name, type, value, p.AsValueString(),
            (read_only ? "only" : "write")+";"), Encoding.Default);

                    string savePath1 = @"d:\test1.txt";
                     
                    System.IO.File.AppendAllText(savePath1,p.AsValueString(), Encoding.Default);
                    /* if(name == "레벨")
                     {
                         value = "테이블 1:1500x750x800mm 2";
                         p.Set(value);
                         // LabUtils.
                     }
             */
        ...
        else foreach( Parameter p in paramsByName )
          {
            ...
            LabUtils.InfoMsg( paramName + ": Name=" + parByNameName
              + "; Type=" + parByNameType + "; Value=" + parByNameValue );
                        ...
                        System.IO.File.WriteAllText(Wfile, paramName + ": Name=" + parByNameName
                          + "; Type=" + parByNameType + "; Value=" + parByNameValue, Encoding.Default);
                    }
      ...
      try
      {
        using( Transaction t = new Transaction( doc ) )
        {
          t.Start( "Bind Fire Rating Shared Parameter" );
                    ...
                    Autodesk.Revit.DB.Binding binding = app.Create.NewInstanceBinding( catSet );
        ...
        try
        {
          ...
          catSet.Insert( doc.Settings.Categories.get_Item(
            BuiltInCategory.OST_ProjectInformation ) );
                    ...
                    Autodesk.Revit.DB.Binding binding = app.Application.Create.NewInstanceBinding( catSet );
